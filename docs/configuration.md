...
Put a configuration file in your project directory: dev/shrinking/files2scan.json. Files for shrinking are split into parts, making this file look like this:
...
```javascript
{
	...
	"part-specific": [
		"browser/some-file-1.js",
		"browser/some-file-2.js"
	],
}
```

Notes:
+ For each part, a .json file with configuration will be auto-generated.
+ In this way you can configure multiple parts, for example to split configuration for generic code and plugins. Based on configuration as above, the file part-generic.json will automatically be updated.
+ For now, only configuration for classes and methods will be auto-updated. If using object syntax as in the example above. **Next planned functionality is auto-updating and shrinking of variables within methods.**
