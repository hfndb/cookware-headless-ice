...
class SassUtils {
    ...
    static beautify(entry) {
        let toReturn = true;
        if (!colorCfg)
            colorCfg = misc_1.Colors.getConfig();
    ...
    static compile(verbose, isWatching = false) {
        let srcDir = path_1.join(cfg.dirProject, cfg.options.sass.dirs.source);
        if (!shelljs_1.test("-d", srcDir)) {
        ...
        files_1.removeObsolete(ro, processed, outDir, ".css");
        if (saydHello && verbose) {
            log.info("... done");
        }
        else if (verbose) {
