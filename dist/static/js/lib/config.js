...
class AppConfig {
    ...
    read() {
        this.options = Object.assign({}, default_config_1.DefaultConfig);
        if (process.env.isNew != undefined) {
            return this.options;
        }
        let settings = path_1.join(this.dirProject, "config.json");
    ...
    static showConfig(options = null) {
        ...
        if (!options)
            options = Object.assign({}, default_config_1.DefaultConfig);
        let project = files_1.FileUtils.readJsonFile(path_1.join(cfg.dirProject, "config.json"), false);
