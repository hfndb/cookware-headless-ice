...
class AppConfig {
    constructor(name) {
        ...
        if (this.isProject && shelljs_1.test("-d", dir)) {
            paths.push(dir);
        }
    ...
    static getProjectDir(dir) {
        while (dir.length > 2 && !shelljs_1.test("-f", path_1.join(dir, "settings.json"))) {
...
class AppMenu {
    ...
    checkOption(toCheck, current) {
        if (process.env.NODE_ENV == "production" || toCheck <= current)
            return;
