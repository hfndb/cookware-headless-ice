...
`);
    ...
    static getLastModifiedDate(path, file) {
        let fullPath = path_1.join(path, file);
        return fs_1.statSync(fullPath).mtime;
    }
    ...
    static getTempFileName(lengthSuffix) {
        return Date.now() + "-" + utils_1.StringExt.getRandom(lengthSuffix);
    }
...
class FileWatcher {
    ...
    addWatch(workingDir, projectDir, path, verbose = true) {
        ...
        if (!shelljs_1.test("-e", fullPath)) {
            log.info(`Path ./${path_1.join(projectDir, path)} doesn't exist. Request to watch ${this.description} ignored`);
            return;
        }
