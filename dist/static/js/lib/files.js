...
`);
    ...
    static getFileList(path, opts = {}) {
        ...
        function addPath(dirname) {
            if (excludeList.includes(path_1.basename(dirname)))
                return;
            fs_1.readdirSync(dirname).forEach((file) => {
                const realpath = path_1.join(dirname, file);
                if (recursive && shelljs_1.test("-d", realpath)) {
                    addPath(realpath);
                }
                else if (shelljs_1.test("-f", realpath)) {
                    addFile(realpath);
                }
            });
        }
...
function removeObsolete(removeObsolete, processed, outputDir, ext) {
    ...
    sources.forEach((file) => {
        if (!object_1.ArrayUtils.inExcludeList(removeObsolete.exclude, file) &&
            ...
            if (shelljs_1.test("-f", path_1.join(outputDir, file, ".map"))) {
                shelljs_1.rm(path_1.join(outputDir, file, ".map"));
            }
