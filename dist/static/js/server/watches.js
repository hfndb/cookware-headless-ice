...
class PhpWatch extends lib_1.FileWatcher {
    change(event, file) {
        event;
        if (path_1.extname(file) != ".php") {
            return;
        }
        if (styling_1.Double.is(file))
            return;
        log.info(`- ${file} changed`);
        let dir = path_1.join(cfg.dirProject, cfg.options.php.dirs.source);
        let session = session_1.SessionVars.getInstance();
        session.add(session_1.ProcessingTypes.php, file);
        let status = new lib_1.FileStatus(dir);
        status.setSoure(file, ".php");
        status.setTarget(dir, ".php");
        php_1.PhpUtils.beautify(status);
    }
}
