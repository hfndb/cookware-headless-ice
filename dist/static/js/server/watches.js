...
class PhpWatch extends _lib.FileWatcher {
  change(event, file) {
    event;

    if ((0, _path.extname)(file) != ".php") {
      return;
    }

    if (_styling.Double.is(file)) return;
    log.info(`- ${file} changed`);
    let dir = (0, _path.join)(cfg.dirProject, cfg.options.php.dirs.source);

    let session = _session.SessionVars.getInstance();

    session.add(_session.ProcessingTypes.php, file);
    let status = new _lib.FileStatus(dir);
    status.setSoure(file, ".php");
    status.setTarget(dir, ".php");

    _javascript.PhpUtils.beautify(status);
  }

}

_defineProperty(PhpWatch, "instance", void 0);
...
function initWatches() {
  ...
  if (cfg.options.php.useWatch) {
    PhpWatch.instance = new PhpWatch(cfg.dirProject, cfg.options.php.dirs.source, "", cfg.options.server.watchTimeout, "Php files");
  }
