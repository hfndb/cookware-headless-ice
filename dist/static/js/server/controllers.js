...
async function controllerGeneric(req, res, next, contentDir, useProjectTemplates, prefix) {
  ...
  if (prefix == "/sys" && url == "index.html") {
    additionalContext = Object.assign(additionalContext, {
      isProject: false,
      systemPackages: _packageJson.Packages.getPackageReadmeFiles(true),
      projectPackages: _packageJson.Packages.getPackageReadmeFiles(false)
    });
  }
  ...
  switch (ext) {
    ...
    case ".html":
      ...
      if (ext != ".md" && req.query && req.query.lint) {
        ...
        if (!data && cfg.options.sys.notifications.compileIssue.html) _sys.SysUtils.notify("Html issue");
